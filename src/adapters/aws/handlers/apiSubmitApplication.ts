...
import { SqsEvaluationQueue } from "../SqsEvaluationQueue";
...
const evaluationQueue = new SqsEvaluationQueue({ queueUrl: requiredEnv("QUEUE_URL") });
...
export async function handler(
  ...
  try {
    const application = await submitApplication(applicationId, { applications, evaluationQueue });

    logger.info("Application submitted", logContext);
    return jsonResponse(200, application);
  } catch (error) {
    logger.error("Failed to submit application", logContext, error);
